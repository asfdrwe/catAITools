...
if [ ! -d koboldcpp ]; then
  result=`osascript -e 'display dialog "koboldcppがインストールされていません。\nプログラムをダウンロードしますか？" with title "ダウンロード" with text buttons {"いいえ","はい"} default button 2'`
...
if [ ! -d models ]; then
  ...
  if [ "$result" = "button returned:はい" ]; then
    mkdir models
    echo "cyberagent-DeepSeek-R1-Distill-Qwen-14B-Japanese-Q4_K_M.gguf"
    ...
    echo "Qwen2.5-VL-7B-Instruct-q5_k_m.gguf"
    ...
    echo "Qwen2.5-VL-7B-Instruct-mmproj-f16.gguf"
